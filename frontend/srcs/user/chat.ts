...
export function initChatSocket(token: string, onReady?: () => void) {
  ...
  if (ws && ws.readyState !== WebSocket.CLOSED) {
    console.log(
      "Closing existing WebSocket connection, readyState:",
      ws.readyState
    );
    
    // Wait for the socket to fully close before creating a new one
    const oldWs = ws;
    ws = null;
    
    const closeHandler = () => {
      oldWs.removeEventListener('close', closeHandler);
      oldWs.removeEventListener('error', errorHandler);
      // Now create the new connection
      createNewWebSocket(token, onReady);
    };
    
    const errorHandler = () => {
      oldWs.removeEventListener('close', closeHandler);
      oldWs.removeEventListener('error', errorHandler);
      // Even on error, create the new connection
      createNewWebSocket(token, onReady);
    };
    
    oldWs.addEventListener('close', closeHandler);
    oldWs.addEventListener('error', errorHandler);
    oldWs.close();
    return;
  }
...
function createNewWebSocket(token: string, onReady?: () => void) {
...
async function loadHistory(friendId: number, messageEl: HTMLElement) {
  ...
  data.messages.forEach((msg: any) => {
    const msgDiv = document.createElement("div");
    if (Number(msg.sender_id) === userId) msgDiv.className = "text-right my-1";
    else msgDiv.className = "text-left my-1";
...
export function openChatWindow(
  ...
  inviteBtn.className =
    "bg-green-500 hover:bg-green-400 text-white px-2 py-1 rounded text-sm";
  ...
  input.className =
    "flex-1 bg-gray-700 text-white rounded px-2 py-1 outline-none";

  const sendBtn = document.createElement("button");
  sendBtn.setAttribute("data-i18n", "send");
  sendBtn.textContent = i18n.t("send");
  sendBtn.className =
    "ml-2 bg-blue-500 hover:bg-blue-400 text-white px-3 py-1 rounded";
